...
*/
	...
	jQuery.fn.smint = function( options ) {
		...
		var lastScrollTop = 0;
		var topFixedOffset = 250; // Aggiunto da Pietro
		//var menuHeight = jQuery(".smint").height(); // Commentato da Pietro
		var menuHeight = -300;
		...
		return jQuery('.smint a').each( function(index) {
			...
			optionLocs.push(Array(jQuery("div."+id).position().top-menuHeight, jQuery("div."+id).height()+jQuery("div."+id).position().top-menuHeight, id));
			
			//Active fisrt menu item (Aggiunto da Pietro)
			if(index == 0) {
			...
			var stickyMenu = function(direction){
				...
				if ((scrollTop + topFixedOffset) > stickyTop) { 
					jQuery('.smint').css({ 'position': 'fixed', 'top':topFixedOffset }).addClass('fxd');	
				} else {
        	...
        	jQuery(this).on('click', function(e){
				...
				var goTo =  jQuery('div.'+ id).offset().top - 50;// -selectorHeight;
